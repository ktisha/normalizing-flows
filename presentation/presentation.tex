...
\begin{frame}{Optimizing the ELBO}
  ...
  \item Just-do-it approach:
    \begin{align*}
      ...
      &= \frac{1}{L} \sum\limits_{l = 1}^L \grad_\theta \log p_\theta(x, z_l) \\
      \grad_\phi \mathcal{L}(\theta, \phi; x)
      &= \grad_\phi \E{q_\phi(z|x)}{\log p_\theta(x, z) - \log q_\phi(z|x)}
       = \op{:(}
    \end{align*}
  \item How to deal with the gradient w.r.t variational parameters $\phi$?
  \end{itemize}
\end{frame}


\begin{frame}{Na\"ive MCMC estimator of $\grad_\phi \mathcal{L}(\theta, \phi; x)$}
