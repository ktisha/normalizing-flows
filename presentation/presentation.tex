...
\begin{frame}[fragile]{Problem scenario}
  \begin{center}
    ...
    \begin{minipage}[t]{.55\linewidth}
      ...
      $p(x, z|\theta) = p(x|z, \theta) p(z)$  %--- joint p.d.f.\\
    \end{minipage}
  \end{center}
...
\begin{frame}{Variational auto-encoder}
  ...
  \begin{itemize}
  \item The parameters $\mu(z)$ and $\sigma^2(z)$ are computed by a neural net,
    which assigns each value of $z$ a \hl{distribution} over $x$.
  \item The parameters $M(x)$ and $S^2(x)$ are computed by a \hl{different}
...
\begin{frame}{VAE illustrated}
  ...
  - \underbrace{\KL{q(z|x, \phi)}{p(z)}}_{\text{regularizer}}
  $$
\end{frame}
