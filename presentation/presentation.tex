...
\begin{frame}{Experiments: Frey faces}
  \begin{figure}
    ...
    \hspace{1em}
    \begin{subfigure}[b]{.4\linewidth}
      \includegraphics[width=\textwidth]{images/vae_frey_B100_E4000_N560_L2_H200_C_sample_64}
      \caption{Samples}
    \end{subfigure}
  \end{figure}
...
\begin{frame}{VAE and normalizing flows}
  ...
  \begin{tikzpicture}
    \node[obs]                 (x1) {$\mathbf{x}$};
    \node[latent, below=of x1] (zK) {$\mathbf{z_K}$};
    \node[latent, below=of zK] (z0) {$\mathbf{z_0}$};
    \node[latent, left=of z0]  (e) {$\epsilon$};
    \node[obs, below=of z0]    (x2) {$\mathbf{x}$};
